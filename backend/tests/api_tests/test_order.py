...
class TestOrder:
    def test_create_order_auth_client(
        self, auth_client, user, shopping_carts, delivery_points
    ):
        order_data = {
        ...
        response = auth_client.post(
            f"/api/users/{user.id}/order/", order_data, format="json"
        )
    ...
    def test_create_order_another_auth_client(
        self, auth_client, user1, shopping_carts, delivery_points
    ):
        order_data = {
            "payment_method": "Payment at the point of delivery",
            "delivery_method": "Point of delivery",
        ...
        response = auth_client.post(
            f"/api/users/{user1.id}/order/", order_data, format="json"
        )
    ...
    def test_delete_order_auth_client(
        ...
        auth_client.post(f"/api/users/{user.id}/order/", order_data, format="json")
        order = Order.objects.get()
        response = auth_client.delete(
            f"/api/users/{user.id}/order/{order.id}/", format="json"
        )
        assert response.status_code == status.HTTP_200_OK

    def test_delete_order_another_auth_client(
        self, auth_client, user1, user, shopping_carts, delivery_points
    ):
        order_data = {
            "payment_method": "Payment at the point of delivery",
            "delivery_method": "Point of delivery",
